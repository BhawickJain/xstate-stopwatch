...
export function Stopwatch(): JSX.Element {
  ...
  return (
    <div id="stopwatch" className={clss("stopwatch", "mt-1")}>
      <div
        className={clss("display", "flex-col", "justify-center")}
        style={displayStyle}
      >
        {formatMillisecondsToReadable(time)}
      </div>
      <div className={clss("control", "flex-row", "flex-gap-1")}>
        <button style={buttonStyle} onClick={() => send("START")}>
          start
        </button>
        <button style={buttonStyle} onClick={() => send("PAUSE")}>
          stop
        </button>
        <button style={buttonStyle} onClick={() => send("RESET")}>
          reset
        </button>
      </div>
    </div>
